...
function updateSubFieldFmt(colId, index, key, val) {
  ...
  if (key === "type") {
    ...
    if (val === "text") {
      delete sub.format.currency;
      delete sub.format.decimals;
    }
  } else {
    sub.format[key] = val;
  }
...
const selectedCellsCount = computed(
  () => (props.block.selectedCells || []).length,
);
...
function mergeSelectedCells(alignCenter = false) {
  ...
  const filteredMerges = mergedCells.filter((m) => {
    ...
    const colOverlap =
      Math.max(newMinColIdx, mMinCol) <= Math.min(newMaxColIdx, mMaxCol);
...
const clientFieldsSorted = computed(() => {
  const order = props.block.fieldOrder || [
    "name",
    "address",
    "phone",
    "email",
...
const companyFieldsSorted = computed(() => {
  ...
  return order.map((key) => ({
    id: key,
    name: companyFieldLabels[key] || key,
  }));
});

function moveField(fromIndex, toIndex) {
  const defaultFields =
    props.block.type === "client_info"
  ...
  <div class="tab-panel">
    ...
    <div v-else-if="block.type === 'signature_line'" class="field-group">
      ...
      <p
        ...
        "
      >
        Edit signature line label, signer name, and date directly on the canvas.
      </p>
      ...
      <div
        v-if="(block.signatureType || 'none') === 'text'"
        style="
          display: flex;
        ...
        <div class="field-row">
          <div>
            ...
            <select
              ...
              class="inp"
              @change="
                updateProp('signatureFont', $event.target.value);
                commitHistory();
              "
            >
      ...
      <div
        v-if="(block.signatureType || 'none') === 'image'"
        style="
          display: flex;
          flex-direction: column;
          gap: 10px;
        ...
        <div>
          ...
          <div style="display: flex; gap: 6px; align-items: center">
            ...
            <button
              ...
              @click="
                updateProp('signatureImage', null);
                commitHistory();
              "
            >
              Clear
        ...
        <div>
          ...
          <div style="display: flex; align-items: center; gap: 10px">
            <input
              ...
              "
              @input="
                updateProp('signatureScale', parseFloat($event.target.value));
                commitHistory();
              "
            />
            <span style="font-size: 11px; min-width: 32px; text-align: right"
              >{{ Math.round((block.signatureScale ?? 1.0) * 100) }}%</span
            >
          </div>
        </div>
      </div>
    ...
    <div v-else-if="block.type === 'watermark'" class="field-group">
      ...
      <p
        ...
        "
      >
        Edit watermark text directly on the canvas.
      </p>
    ...
    <div v-else-if="block.type === 'company_info'" class="field-group">
      ...
      <p
        ...
        "
      >
        Edit company details directly on the canvas.
      </p>
      ...
      <div style="display: flex; flex-direction: column; gap: 10px">
        <div
          v-for="field in ['Name', 'Address', 'Phone', 'Email', 'Website']"
          :key="field"
          style="
            display: flex;
            align-items: center;
    ...
    <div v-else-if="block.type === 'client_info'" class="field-group">
      ...
      <p
        ...
        "
      >
        Edit client details directly on the canvas.
      </p>
      ...
      <div style="display: flex; flex-direction: column; gap: 10px">
        <div
          v-for="field in ['Name', 'Address', 'Phone', 'Email', 'TaxId']"
          :key="field"
          style="
            display: flex;
            align-items: center;
          ...
          "
        >
          <span>Show Client {{ field === "TaxId" ? "Tax ID" : field }}</span>
          <label class="toggle">
            <input
              type="checkbox"
              :checked="block[`show${field}`] !== false"
    ...
    <div v-else-if="block.type === 'bank_details'" class="field-group">
      ...
      <p
        ...
        "
      >
        Edit bank details directly on the canvas.
      </p>
      <div class="divider" />
      ...
      <div style="display: flex; flex-direction: column; gap: 10px">
        <div
          ...
          "
        >
          <span>Show {{ field.replace(/([A-Z])/g, " $1").trim() }}</span>
          <label class="toggle">
            <input
              type="checkbox"
              :checked="block[`show${field}`] !== false"
    ...
    <div v-else-if="block.type === 'item_table'" class="field-group">
        ...
        <div>
          <label
            style="
              ...
              margin-bottom: 2px;
            "
            >Default Row Height</label
          >
          <div class="field-unit">
            <input
              type="number"
              :value="block.defaultRowHeight ?? 30"
              class="inp"
              min="16"
              max="120"
              @input="handleInput('defaultRowHeight', $event)"
              @blur="commitHistory"
            />
            <span class="field-unit-label">px</span>
          </div>
      ...
      <div class="field-row" style="margin-top: 8px">
        <div>
          ...
          <select
            ...
            class="inp"
            @change="
              updateProp('fontFamily', $event.target.value);
              commitHistory();
            "
          >
      ...
      <div class="field-row" style="margin-top: 8px; margin-bottom: 8px">
        <div>
          ...
          <select
            ...
            class="inp"
            @change="
              updateProp('borderStyle', $event.target.value);
              commitHistory();
            "
          >
            <option value="solid">Solid</option>
      ...
      >
        ...
        >
          ...
          <select
            :value="
              block.emptyRowBordersMode ??
              (block.showEmptyRowBorders === false ? 'none' : 'grid')
            "
            class="inp"
            style="width: 110px; padding: 2px 4px; font-size: 11px"
        ...
        <div>
          ...
          <div style="display: flex; gap: 6px; align-items: center">
            <input
              type="color"
              :value="
                block.headerColor && block.headerColor.startsWith('#')
                  ? block.headerColor
                  : '#333333'
              "
              class="color-picker-input"
              @input="updateProp('headerColor', $event.target.value)"
              @change="commitHistory"
            />
            <input
              type="text"
      ...
      >
        ...
        <label class="toggle">
          <input
            type="checkbox"
            :checked="(block.headerFontWeight ?? 'bold') === 'bold'"
            @change="
              updateProp(
                'headerFontWeight',
      ...
      <div class="field-row" style="margin-top: 10px">
        <div>
          ...
          <select
            ...
            class="inp"
            @change="
              updateProp('headerFontFamily', $event.target.value);
              commitHistory();
            "
          >
            <option value="inherit">Use Global Font</option>
        ...
        >
          ...
          <div style="display: flex; align-items: center; gap: 6px">
            ...
            <span
              style="
                font-size: 12px;
                color: var(--color-panel-muted, #666);
                cursor: grab;
                user-select: none;
                flex-shrink: 0;
              "
            ...
            <input
              ...
              @change="
                updateColumnProp(col.id, 'visible', $event.target.checked)
              "
            />
          ...
          >
            ...
            <template
              v-if="
                getColumnFormat(col).type === 'number' ||
                getColumnFormat(col).type === 'currency'
              "
            >
              <div
                class="field-unit"
                style="width: 48px; height: 22px; flex: 0 0 auto"
              >
                <input
                  type="text"
                  inputmode="numeric"
            ...
            <div style="margin-top: 4px; padding-left: 2px">
              <div
                style="
                  ...
                  user-select: none;
                "
                @click="toggleSubFields(col.id)"
              >
                <span style="font-size: 9px; color: var(--color-panel-muted)">{{
                  col.subFieldsOpen ? "▼" : "▶"
                }}</span>
                <span
                  style="
                    ...
                    color: var(--color-panel-muted);
                  "
                  >Sub-Fields ({{ (col.subFields || []).length }})</span
                >
                <span
                  v-if="col.subFields?.length"
                  style="
                    font-size: 8px;
                    ...
                    cursor: pointer;
                  "
                  @click.stop="clearSubFields(col.id)"
                  >Clear</span
                >
              </div>
              <div
                v-if="col.subFieldsOpen"
                style="
                  display: flex;
                  flex-direction: column;
                  gap: 4px;
              ...
              >
                ...
                >
                  <div style="display: flex; align-items: center; gap: 4px">
                    ...
                    <select
                      ...
                      "
                      @change="
                        handleSubDataKeyChange(col.id, si, $event.target.value)
                      "
                    >
                      <option
                        v-for="key in availableDataKeys"
                        :key="key"
                        :value="key"
                      >
                        {{ key }}
                    ...
                    <input
                      v-if="
                        isCustomSubKey(sub.dataKey) ||
                        showCustomSubKeyMap[`${col.id}_${si}`]
                      "
                      type="text"
                      :value="sub.dataKey ?? ''"
                      class="inp"
                    ...
                    <div
                      class="field-unit"
                      style="height: 18px; flex: 0 0 auto"
                    >
                      <input
                        type="number"
                        min="6"
                    ...
                    <input
                      ...
                      "
                      @input="
                        updateSubField(col.id, si, 'color', $event.target.value)
                      "
                    />
                    <button
                      class="btn btn-ghost btn-icon text-danger"
                      ...
                      @click="removeSubField(col.id, si)"
                    >
                      ×
                    </button>
                  </div>
                  ...
                  >
                    ...
                    <template
                      v-if="(sub.format?.type ?? 'text') === 'currency'"
                    >
                      <input
                        type="text"
                    ...
                    <template
                      v-if="
                        (sub.format?.type ?? 'text') === 'number' ||
                        sub.format?.type === 'currency'
                      "
                    >
                      <div
                        class="field-unit"
                ...
                <button
                  class="btn btn-ghost"
                  style="font-size: 9px; padding: 2px 0; width: 100%"
                  @click="addSubField(col.id)"
                >
                  + Add Sub-Field
                </button>
              </div>
    ...
    <div v-else-if="block.type === 'checkboxes_row'" class="field-group">
      ...
      >
        <div
          ...
          "
        >
          <span style="font-size: 11px; font-weight: 500; flex: 1">{{
            opt.label || "Option"
          }}</span>
          <!-- Default checked -->
          <label
          ...
          >
            <input
              ...
              :checked="opt.checked"
              @change="
                updateCheckboxOption(index, 'checked', $event.target.checked);
                commitHistory();
              "
            />
            Pre-check
    ...
    <div v-else-if="block.type === 'barcode'" class="field-group">
      ...
      <input
        ...
        @input="
          blockStore.updateBlock(block.id, { content: $event.target.value })
        "
        @change="commitHistory"
      />
      ...
      <input
        ...
        @change="
          blockStore.updateBlock(block.id, {
            barcodeWidth: parseFloat($event.target.value) || 2,
          });
          commitHistory();
        "
      />
      ...
      <input
        ...
        @change="
          blockStore.updateBlock(block.id, {
            barcodeHeight: parseFloat($event.target.value) || 50,
          });
          commitHistory();
        "
      />
      ...
      <label class="toggle">
        <input
          ...
          @change="
            blockStore.updateBlock(block.id, {
              showBarcodeText: $event.target.checked,
            });
            commitHistory();
          "
        />
      ...
      <input
        ...
        @change="
          blockStore.updateBlock(block.id, {
            barcodeFontSize: parseFloat($event.target.value) || 12,
          });
          commitHistory();
        "
      />
    ...
    <div v-else-if="block.type === 'carbon_copy_label'" class="field-group">
      ...
      <p style="font-size: 11px; color: var(--color-panel-muted); margin: 0">
        Edit copy label text directly on the canvas.
      </p>
    </div>
    ...
    <div v-else-if="block.type === 'spacer'" class="field-group">
      ...
      <div style="margin-top: 10px">
        <input
          ...
          style="
            width: 100%;
            accent-color: var(--color-accent, #00b4d8);
            cursor: pointer;
          "
          @input="handleInput('height', $event)"
          @change="commitHistory"
